...
dc1 <- dc %>% summarise(
  ...
  `Excluded (Overall)`                       = sum(consented == 3 | is.na(consented), na.rm = T),
  ...
  `Refused - Outside antibiotic use`         = sum(refused_antibiotics == 1, na.rm = T),
  `Refused - Did not want placebo`           = sum(refused_placebo == 1, na.rm = T),
  `Refused - Other`                          = sum(refused_other == 1, na.rm = T),
  `Refused - No reason given`                = sum(consented == 2 & refused_antibiotics == 0 & 
                                                     refused_placebo == 0 & refused_other == 0, na.rm = T),
...
kable(dc1) %>% 
  row_spec(c(2, 14, 20), bold = T, italic = T)
```
